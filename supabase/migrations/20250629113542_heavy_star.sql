/*
  ...
  2. Purpose
    ...
    - Support efficient database-level filtering for the frontend
...
WHERE i.categories IS NOT NULL 
  AND json_typeof(i.categories) = 'array'
  AND json_array_length(i.categories) > 0
  AND json_array_elements_text(i.categories) IS NOT NULL
  AND trim(json_array_elements_text(i.categories)) != '';
